...
def parse_model_config(path):
    ...
    for line in lines:
        ...
        #     module_defs[-1][key.rstrip()] = value.strip()
            ...
            # Check if the line contains at least one '=' character
            key, value = line.split("=", 1)  # Split only at the first '='
            module_defs[-1][key.rstrip()] = value.strip()
        # else:
        #     # Handle lines that don't contain '=' (you might want to log or handle them differently)
        #     print(f"Warning: Line skipped - {line}")
    print(module_defs)
    return module_defs
