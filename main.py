...
OPTIMIZED_MODEL_PATH= "/".join((BASE_DIR, POSE_MODEL_DIR, OPTIMIZED_MODEL))
MODEL_WEIGHTS_PATH= "/".join((BASE_DIR, POSE_MODEL_DIR, MODEL_WEIGHTS))

if os.path.exists(OPTIMIZED_MODEL_PATH) == False:
...
'''
def execute_2(img, org): # v2Img, pilImg
    start = time.time()
...
out_video = cv2.VideoWriter('result_%s_%s.mp4'%(args.model, name), fourcc, cap.get(cv2.CAP_PROP_FPS), (W, H))
count = 0
